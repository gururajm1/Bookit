  ...
  useEffect(() => {
    const fetchBookedTickets = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:1004/bookit/user/booked-tickets?email=${encodeURIComponent(userEmail || JSON.parse(localStorage.getItem('user') || '{}').email)}`);
