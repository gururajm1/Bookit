...
const Payment = () => {
  ...
  const handlePaymentSuccess = async (paymentResponse: any) => {
    try {
      ...
      // Update cinema seats
      const updateResponse = await fetch('http://localhost:1002/bookit/cinema/seats', {
        method: 'POST',
        headers: {
      ...
      const userUpdateResponse = await fetch('http://localhost:1002/bookit/user/add-booking', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          ...
          booking: {
            movieName: paymentData.movieName,
            movieCertification: 'NA',
            genres: 'NA',
            language: 'NA',
            theatreName: paymentData.theatreName,
            theatreLocation: paymentData.theatreLocation,
            showDate: paymentData.showDate,
            showTime: paymentData.showTime,
            totalAmount: paymentData.totalAmount,
            selectedSeats: paymentData.selectedSeats.split(',').map(seat => seat.trim()),
